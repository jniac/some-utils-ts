...
export function formatNumber(n, { maxDigits = 8, } = {}) {
    if (maxDigits < 6) {
    ...
    const usePrecision = ilen > maxDigits ||
        flen < 0;
    if (usePrecision) {
    ...
    const useExpontential = Math.abs(n) < 1 / Math.pow(10, maxDigits - 2);
    if (useExpontential) {
...
export function formatBigNumber(n, { thousandsSeparator = ',', } = {}) {
