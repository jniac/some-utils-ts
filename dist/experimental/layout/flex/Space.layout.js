...
export function computeRootRect(space) {
    ...
    space.rect.set(offsetX.value, offsetY.value, sizeX.value, sizeY.value);
}
const _padding = new Padding();
export function computePadding(space) {
    ...
    return _padding.setTRBL(space.padding[0].compute(h, w), space.padding[1].compute(w, h), space.padding[2].compute(h, w), space.padding[3].compute(w, h));
}
const _childrenMargins = [];
function computeChildrenMargins(space) {
    ...
    for (let i = 0; i < space.children.length; i++) {
        ...
        _childrenMargins[i].setTRBL(child.margin[0].compute(h, w), child.margin[1].compute(w, h), child.margin[2].compute(h, w), child.margin[3].compute(w, h));
    }
}
function computeSize(space, width, height, direction, margin) {
    let w = 0, h = 0;
...
export function computeChildrenRect(space) {
    ...
    const { x: innerX, y: innerY, width: innerWidth, height: innerHeight, } = _innerRect
        .copy(space.rect)
    ...
    const tangentSpacings = Array.from({ length: flowChildren.length + 1 });
