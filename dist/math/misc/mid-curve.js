...
export const computeMidBase = (m) => {
    m = 1 / m - 1;
    return m * m;
};
export const mid = (x, m) => {
    const b = computeMidBase(m);
    return (b ** x - 1) / (b - 1);
};
export const midInverse = (x, m) => {
    const b = computeMidBase(m);
 ...
 */
export const interpolateWithMidPoint = (min, max, midValue, t) => {
    const d = max - min;
 ...
 */
export const interpolateWithMidPointUnclamped = (min, max, midValue, t) => {
    const d = max - min;
    const m = (midValue - min) / d;
    if (m <= 0 || m >= 1) {
        // Invalid "mid" value, lerp fallback
 ...
 */
export const inverseInterpolateWithMidPoint = (min, max, midValue, x) => {
    const d = max - min;
    const m = (midValue - min) / d;
 ...
 */
export const inverseInterpolateWithMidPointUnclamped = (min, max, midValue, x) => {
    const d = max - min;
    const m = (midValue - min) / d;
    const t = (x - min) / d;
    if (m <= 0 || m >= 1) {
