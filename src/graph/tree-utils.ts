...
type ParentAccessor<T> = (node: T) => T | null | undefined
...
type AllAscendantsOfOptions<T> = Partial<ReturnType<typeof allAscendantsOfOptionsDefaults<T>>>

export function isAncestorOf<T>(ancestor: T, descendant: T, options: Partial<{ includeAncestor: boolean, getParent: ParentAccessor<T> }> = {}): boolean {
  const {
    includeAncestor = true,
    getParent = (node => (node as any).parent) as ParentAccessor<T>,
  } = options
  let current: T | null | undefined = descendant

  if (includeAncestor === false && current === ancestor)
    return false
...
export function* allAscendantsOf<T>(firstNode: T, options: AllAscendantsOfOptions<T> = {}): Generator<{ node: T }> {
  const {
    nodesMax,
    includeFirstNode,
    filter,
  ...
  let current: T | null | undefined = includeFirstNode ? firstNode : getParent(firstNode)
