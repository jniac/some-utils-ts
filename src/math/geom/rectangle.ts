...
export function fromRectangleDeclaration(declaration: RectangleDeclaration, out = new Rectangle()) {
  ...
  if ('extent' in restDeclaration) {
    const { position = 0, center = position, extent } = restDeclaration as any
...
function safeCollapse<T extends RectangleLike>(rect: RectangleLike, out: T): T {
...
function safeFlip<T extends RectangleLike>(rect: RectangleLike, out: T): T {
  const { x, y, width, height } = rect
  if (width < 0) {
    out.x = x + width
    out.width = -width
...
function toBoundingInt<T extends RectangleLike>(rect: RectangleLike, out: T): T {
  const { x, y, width, height } = rect
...
export class Rectangle implements RectangleLike, Iterable<number> {
  ...
  *[Symbol.iterator](): Generator<number> {
  ...
  static applyPaddingDefaultOptions = {
     ...
     */
    safeMode: <'collapse' | 'flip' | 'none'>'collapse',
  }
  ...
  applyPadding(padding: PaddingDeclaration, options: unknown): this {
    const { mode, safeMode } = {
